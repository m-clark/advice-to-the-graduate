...
Search for and Click your dependent variable
...
```{r, eval=F}
d = haven::read_spss('filelocation')
model = lm(DV ~ IV1 + IV2 + IV3, data=d)
```

There is no debate here, one is vastly more efficient and simply easier.  You may have to learn that you can use the <span class="pack">haven</span> package for importing SPSS files, or the <span class="func">lm</span> function to do regression, but that's very quickly picked up, and less than you would have had to learn regarding the SPSS menus.  The other nice thing is that you can redo the above in two steps with Ctrl+A (select all) and Ctrl+Enter (run).
...
However, assuming you want others to be able to use your code, you should feel bad giving them shitty code.  Going back to one of our initial suggestions, *Don't be a jerk*. Write better code, always. Time and other resources will always be working against you, but do the best you can.
